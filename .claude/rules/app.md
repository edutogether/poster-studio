...
- 운영 규모(2026-09-09 대표 확정):
  ...
  - **"노트북 20대"는 배치 계획이 아니라 부하 테스트 목표치다.** 대표 표현으로
    ...
    **문서만 읽고 20대를 실제 배치로 오해한 사례가 실제로 있었다**(2026-09-09 팀장) —
    그래서 여기 명시해 둔다.
  - `IP_RATE_LIMIT_MAX=50`은 **공인 IP 3개**를 전제로 한 값이다(50 = 전역 150 ÷ 3).
    노트북 대수가 아니라 **IP 개수**에서 나온 숫자이므로 3대 확정으로 값이 바뀌지 않는다.
    바뀌어야 하는 조건은 **공인 IP 개수가 3이 아니게 되는 것**이다 — 그 경우
    `functions/index.js`의 근거 주석과 함께 다시 산정한다.
