...
// (public/app.js의 genCount/MAX_GENERATIONS_PER_PHOTO)을 회귀 테스트로 고정한다.
// 5차 감사 발견: 이전엔 브라우저에서 fetch를 목(mock)으로 바꿔 수동으로 한 번
// 확인한 게 전부였고, 자동화된 회귀 테스트가 없어 다음 수정 때 조용히 깨질 수
// 있었다. __eval로 app.js의 let 바인딩(capturedBlob, genCount)에 직접 접근해
// 실제 generateBtn/regenBtn 클릭 핸들러를 그대로 실행시켜 검증한다.
...
test('genCount: 1차 생성 성공 후 재생성 버튼이 활성 상태를 유지한다', async () => {
  ...
  app.__eval("capturedBlob = { size: 3 };");

  await app.document.getElementById('generateBtn').onclick();

  assert.equal(app.__eval('genCount'), 1);
...
test('genCount: 2차(재생성) 후에는 재생성 버튼이 비활성화된다', async () => {
  ...
  assert.equal(app.__eval('genCount'), 2);
...
test('genCount: 다시 촬영하면(retakeBtn) 카운트가 초기화되고 재생성 버튼이 다시 활성화된다', async () => {
  ...
  assert.equal(app.__eval('genCount'), 0);
  assert.equal(app.document.getElementById('regenBtn').disabled, false);
});

test('genCount: regenBtn은 결과가 없으면(posters 비어있음) 아무 동작도 안 한다', () => {
  const app = loadApp();
