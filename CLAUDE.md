...
| 4 | 테스트 커버리지 | 95 | −5: OpenAI 실호출 코드 의도적 미테스트(구조적, 옳은 설계). TEMPLATES 픽셀검증 불가(−2)는 @napi-rs/canvas 실캔버스 테스트 4종 추가로 해소. **3번과 달리 유지**: 이건 "미래의 가정"이 아니라 지금 이 순간 자동검증이 안 되는 코드가 실재하는 것(대표 확인) |
...
| **평균** | | **99.5**(2·3·7·10번 재검토 반영 — 감점 근거가 "포기한 것 없음"·"미래 가정"·"코드가 이미 막고 있음"인 항목은 100으로 정정, "지금 실재하는 검증 공백"인 4번만 유지) | |
...
**4번(테스트 커버리지) — 개선 방법 검토, 아직 미착수(승인 대기).** "OpenAI를 실제로 호출하지 않고, 응답을 흉내 낸 가짜(mock)로 우리 쪽 로직만 검증할 수 있는가?"라는 질문에 대한 답: **가능하고, 비용도 안 든다.** `functions/index.js`의 `_client`(OpenAI SDK 인스턴스, `getClient()`로 lazy 생성)를 Firestore 카운터 때 썼던 것과 같은 방식(`_setCounterImplForTesting`과 동일 패턴의 `_setClientForTesting(fakeClient)`)으로 테스트에서 갈아끼울 수 있는 구조라, 실제로 다음이 가능하다:
- `editWithRetry`의 429/5xx 재시도 로직(지금은 완전 미검증) — 가짜 클라이언트가 처음 N번 429를 던지다 성공하는 시나리오로 재시도 횟수·백오프 동작 검증.
- `generateArt`의 파라미터 폴백 로직(`input_fidelity`/`quality` 거부 시 재시도) — 가짜 클라이언트가 특정 파라미터에서만 400을 던지도록 해서 폴백 경로 검증.
- 결과가 비어있을 때(`imgs.length===0`) 에러를 던지는 경로.
- `checkOpenAIReachable()`의 60초 캐싱 동작(현재 이 캐싱 로직 자체가 테스트 0개) — 가짜 클라이언트로 성공/실패/캐시 재사용 시나리오 검증.
이건 실제 OpenAI 서버 호출을 여전히 전혀 안 하므로(zero-dependency·무비용 원칙 그대로 유지) 실비용이 들지 않는다.
**한계(대표가 이미 인정한 부분)**: 이렇게 해도 "우리 로직이 맞다"만 검증되지, OpenAI의 실제 API 응답 형식·타이밍·에러 코드가 가짜 클라이언트의 가정과 정확히 같은지는 여전히 검증 못 한다 — 그 갭은 구조적으로 남는다(3번을 100으로 정정한 것과 같은 "미래 가정" 성격이 아니라, "지금 이 순간 이 특정 코드 경로가 실제 API와 한 번도 안 맞대본 적 없다"는 실재하는 한계라 대표가 4번을 3번과 다르게 유지시킨 논리와 일치함).
**예상 효과**: 지금 -5(95점)인 사유 중 "재시도·폴백·에러매핑 로직이 전혀 안 맞음"이라는 부분은 위 테스트로 해소되지만, "실제 OpenAI와 한 번도 안 맞대봄"이라는 근본 한계는 구조적으로 남는다 — 대표가 3번 판단 때 쓴 기준을 그대로 적용하면 대략 **95 → 97~98점** 사이가 합리적인 추정(정확한 값은 감점 기준을 정하는 대표 판단 영역).
**예상 소요시간**: 약 1~1.5시간(`_setClientForTesting` 시딩 10분, 가짜 클라이언트+시나리오별 테스트 6~8개 작성 50~70분, 검증 10분).
**승인 대기 — 아직 코드 작업 시작 안 함.**

**최종 평균 99.5** — 위 표(2·3·7·10번 100점 정정) 참고.
