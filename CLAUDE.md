...
- **상태**: **실운영 모드**(아래 섹션 참고). **9차 종합감사 10/10(2026-09-10)** —
  점수는 **`10/10`으로만 적는다**(2026-09-10 대표 지시, `COMMON_STANDARDS.md` §26과 같은 취지).
  채점은 org 공통 10개 항목이고 항목마다 100점 만점이라 **10개 전부 100점 = `10/10`**이다.
  `100/100`·`만점` 같은 다른 표기를 쓰지 않는다 — 여덟 저장소를 한 표에 놓을 때 사람이 매번
  번역해서 읽게 되기 때문이다. React
