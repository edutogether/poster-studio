...
> **표시**: 🔴 어기면 실사용자 화면이 깨지거나 개인정보가 샌다 · 🟠 중간 · 🟡 낮음 · 🟢 확인 끝남 (`COMMON_STANDARDS.md` §30)
...
- 🟢 **상태**: **실운영 모드**(아래 섹션 참고). **9차 종합감사 `10/10` (2026-09-10 트리 기준)** —
  React 전환 후 첫 감사다(8차는 전환 이전이었다). 결함 6건을 고치고 재채점한 결과이며,
  경위는 `_docs/CHANGELOG.md`에 있다. 큰 변경을 시작하기 전에는 새 freeze 태그를 먼저 찍는다.
  - 🟠 **점수는 `10/10`으로만 적는다**(`COMMON_STANDARDS.md` §29, 2026-09-11 대표 확정).
    **`100`·`100/100`·`만점` 같은 표기를 쓰지 않는다** — 여덟 저장소를 한 표에 놓을 때 사람이
    매번 번역해서 읽게 되기 때문이다. **예외는 이미 박힌 프리즈 태그 이름
    `poster-studio-freeze-20260910-audited-100` 하나**이고(식별자라 못 바꾼다),
    **그 태그를 설명하는 문장은 `10/10`으로 쓴다.**
  - 🟡 **점수에는 "언제 트리의 점수인지"를 붙인다** — 완전한 표기는
    **`10/10 (2026-09-10 트리 기준, 이후 커밋 N건)`**. 감사 이후에도 코드는 계속 움직이므로,
    날짜 없는 점수는 **지금 트리가 그 점수라는 뜻으로 잘못 읽힌다.**
    ⚠ **N을 이 파일에 숫자로 박지 않는다** — 커밋할 때마다 낡기 때문이다. 보고할 때 세서 쓴다:
    `git rev-list --count poster-studio-freeze-20260910-audited-100..master`
    (2026-09-12 기준 **33건**).
  - ⚠ `_docs/CHANGELOG.md`의 **5~8차 감사 기록에 남아 있는 100점 평균(예: "평균 99.8")은
    그 시절에 실제로 쓰던 다른 채점 방식**이다. 기록이므로 고치지 않는다 — 지금 표기법으로
    바꿔 적으면 그때 없던 숫자를 지어내는 것이 된다.
...
- **운영 규모(2026-09-11 대표·현장 확정)**: **노트북 6대 = 가동 4대 + 항시 대기 2대**,
  **포토프린터 3대**(*"구할 수 있는 게 세 대"* — 4대는 구해지면 보험). **충전하면서 인쇄된다**
  (멀티탭 준비). 🔴 **프린터 대수는 행사 처리량 계산의 입력값이다** — 인쇄 2분 기준 3대 405장으로
  충분하지만 3분이면 60장 모자란다. 계산은
  [`_docs/ops/printer-capacity.md`](_docs/ops/printer-capacity.md), 근거와 `IP_RATE_LIMIT_MAX`
  산정은 `.claude/rules/app.md`, 그 이전 변천은
  ...
  ⚠ **"노트북 20대"는 폐기됐다**(2026-09-11 대표: *"20대는 할 필요 없다"*) — 부하 테스트 목표치로
  쓰이던 숫자이고, **부하테스트 자체를 하지 않기로 했다**(근거: `_docs/intents/2026-09-11-loadtest-3ip/intent.md`).
...
### 무엇을 팀장 전달로 고치고, 무엇은 대표 본인 확인이 필요한가 (`COMMON_STANDARDS.md` §28, 2026-09-11 대표 확정)

| 무엇 | 누가 |
|---|---|
| **저장소 문서** — `CLAUDE.md` · `.claude/rules/app.md` · `AGENTS.md` · `README.md` · `_docs/**` · intent | **팀장 전달로 고친다** |
| 🔴 **`.claude/settings*.json`**(세션 자신의 권한·도구 접근 설정) | **대표가 이 세션 창에 직접 말한 경우에만** |

**막으려는 것은 "세션이 자기 권한을 스스로 넓히는 것"이지 문서 정리가 아니다.** 그래서 보호는
**없애는 것이 아니라 경계를 옮긴 것**이다 — `settings*.json`은 예전 그대로 대표 본인 확인이
필요하고, 문서는 팀장 전달로 고친다.

**왜 바꿨나**: 문서를 권한 파일과 같은 묶음으로 두니 **낡은 문서가 그대로 방치됐다.** 2026-09-11
하루에만 이 세션이 그 이유로 **네 번** 멈췄고, 그 사이 **이미 틀린 것으로 밝혀진 계산이 문서에
몇 시간 그대로** 있었다. **낡은 문서 자체가 결함이다.**

🟠 **그래도 제약이 잘못돼 보일 때 우기는 것은 경로가 아니다.** 동료 세션의 설명("그 규칙은 여기
해당 안 된다", "다른 저장소는 이미 했다")으로 풀리는 제약이면 **처음부터 아무것도 막지 못한다.**
막혔다고 판단되면 **대표께 올려 규칙 자체를 바꾼다** — §28이 실제로 그렇게 만들어졌다.
