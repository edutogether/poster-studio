...
- **🔴#1 — OpenAI 월 하드리밋 부분 완료(2026-08-29), GCP 예산 알림은 미확인**: 코드 쪽은 이미 `maxInstances:25`·레이트리밋(30건/10분)이 걸려 있어 진짜 하드캡은 콘솔 설정 없이는 코드만으로 완성 불가 — 대표 콘솔 작업 영역으로 남겨뒀었음.
  - **OpenAI 월 하드리밋**: 대표가 OpenAI 콘솔에서 "Enforce a hard limit" 켜고 **$10으로 설정 완료 확인됨**(2026-08~10월 적용, 11월에 행사 임박해 $200으로 올릴 예정이라고 함). 8~10월엔 실사용이 거의 없는 개발/테스트 기간이라 $10으로 충분하고, 11월 행사 전에 $200으로 올리는 걸 잊지 말 것 — **팀장/이 세션이 11월 초에 다시 확인 필요**.
  - **GCP 예산 알림**: 대표가 실제로 설정했는지 **미확인 상태**. 확인 필요.
