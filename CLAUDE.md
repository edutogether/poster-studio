...
| # | 항목 | 점수 | 남은 감점 사유 |
|---|---|---|---|
| 1 | 코드 품질/일관성 | 99 | 490줄 단일파일 문제 해소(분리 완료) — 잔여는 사소한 스타일 수준 |
| 2 | 기술 부채 | 96 | −4: Google Fonts CDN 외부의존(대표 판단으로 계속 보류) |
| 3 | 에러 핸들링 | 98 | −2: 문자열 패턴 매칭 오류분류의 미래 API 변경 잔여 리스크(변동 없음) |
| 4 | 테스트 커버리지 | 93 | −5: OpenAI 실호출 코드 의도적 미테스트(구조적, 옳은 설계) / −2: TEMPLATES 테스트가 픽셀까지는 검증 못함(변동 없음) |
| 5 | 아키텍처/구조 설계 | 99 | 490줄 문제 해소(1번과 동일 원인 공유) |
| 6 | 확장성 | **100** | 진짜 동시요청 160건(순차 아님) 실측 완료 — 정확히 150건 통과·10건 차단, 오차 0. Firestore 트랜잭션이 실제 경합에서도 정확히 직렬화됨을 증명 |
| 7 | 운영 안정성 | 97 | −3: 콜드스타트 완화(`minInstances`) 대표가 명확히 거부 확정(상시비용 대비 불필요 판단) — 변동 없음 |
...
**진짜로 열려있는 질문(대표 판단 필요, 아직 아무도 결정 안 함)**: Google Fonts 자체호스팅 여부(2번 기술부채 −4 원인, 진행 시 2~3시간+), 재생성 한도를 2회→1회로 줄여 비용 여유를 늘릴지(10번 −4 원인, UX 트레이드오프).

**미해결이지만 결정 불필요/구조적인 것(대표가 뭘 승인할 사안 아님)**: 1·5번의 파일 로드순서 암묵적 의존(−1), 3번의 미래 OpenAI 오류형식 변화 리스크(−2), 4번의 OpenAI 실호출 코드 의도적 미테스트(−5, 옳은 설계), 4번의 TEMPLATES 픽셀검증 불가(−2, 이 작업 환경에 캔버스 네이티브 빌드툴이 없어서 — 대표 컴퓨터 문제 아님), 10번의 25% 예산여유(−4, 이미 확정된 다른 숫자들의 산술적 결과일 뿐).

**완료(2026-08-30 추가)**: 6·8번 동시경합 부하테스트 — 진짜 동시 160건(순차 아님) 발사해 정확히 150통과/10차단(오차 0) 확인, 두 항목 다 100점.

**7번 콜드스타트 추가 조사(대표 요청, 결정 대기)**:
- **Startup CPU Boost**: 시작 순간에만 CPU를 더 줘서 콜드스타트 자체를 단축하는 기능, 거의 무료. **Cloud Functions 2세대는 기본적으로 이미 켜져 있음(구글 공식) — 추가 조치 불필요, 이미 적용중.**
- **Cloud Scheduler 주기적 핑(예: 5분 간격)**: `minInstances` 상시유지보다 훨씬 저렴(사실상 무료)하게 "0→1" 콜드스타트를 줄이는 절충안. 한계: 이미 떠있는 상태에서 갑자기 여러 명 몰리는 "1→N" 스케일업 콜드스타트는 못 막음(노트북 대수가 적어 이 시나리오 영향은 제한적일 것), Cloud Run이 마지막 요청 후 1분 안에도 내릴 수 있어 100% 보장은 아님. 행사 당일만 켜는 방식으로 대표 판단 대기.
- 함수 초기화 코드 자체는 이미 가벼움(OpenAI 클라이언트 lazy 초기화 등) — 추가로 줄일 게 별로 없음.

**1·5번 ES모듈 전환 — 결정 대기, 정확한 범위 재산정**: 단순 export/import 추가는 기계적이라 빠르지만, `stream`/`capturedBlob`/`genCount` 등 여러 파일이 공유하는 가변 상태는 ES모듈에서 직접 재할당이 안 돼 구조 변경(공유 객체 또는 getter/setter)이 필요함 + 테스트 인프라(`load-app.js`) 전면 재작성 필요. **처음 추정한 1.5~2h는 "파일 분리"(이미 완료) 기준이었고, ES모듈 전환은 이 위에 얹는 별개의 더 큰 작업 — 현실적 재추정 2.5~3.5h.**

**4번 TEMPLATES 픽셀테스트 — 새로운 발견, 결정 대기**: `canvas` npm 패키지가 Visual Studio C++ 빌드툴 없어서 설치 실패했던 건 맞지만(`Could not find any Visual Studio installation`), **컴파일이 필요 없는 대안 `@napi-rs/canvas`(미리 빌드된 바이너리)는 방금 실제로 설치·한글 렌더링까지 성공 확인함.** 즉 이 −2는 실제로 닫을 수 있음 — 진짜 캔버스 테스트 인프라로 재작성 시 약 1~1.5h 추정.

**2번 Google Fonts 자체호스팅 — 결정 대기**: 자체호스팅 시 외부 CDN/학교망 차단 리스크 제거되지만 서브셋팅 없이 폰트 파일이 수십MB로 커짐(2~3h+ 소요). 지금까지 실제 차단 사례 없어서 현행 유지를 권장하나 최종 대표 판단 사안.
