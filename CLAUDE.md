...
- **🔴#1, #4 — 코드로 손 못 대는 영역, 대표 판단/콘솔 작업 대기**:
  - #1(비용 상한): OpenAI 대시보드 월 하드리밋 + GCP 예산 알림은 대표 콘솔 작업 영역. 코드 쪽은 이미 `maxInstances:5`·레이트리밋(10건/10분)이 걸려 있어 추가로 더 조일 경우 오히려 🟡 "레이트리밋이 실수요보다 낮음" 항목과 충돌하므로, 진짜 하드캡은 콘솔 설정 없이는 코드만으로 완성 불가하다고 판단해 추가 코드 변경 안 함.
  - #4(PNG 저장 버튼): 제거 vs 행사 후 삭제 체크리스트, 두 옵션 다 대표 판단 필요라 아직 미착수.
