...
| 6 | 확장성 | 97 | −3: Firestore 전역화는 완료했지만 진짜 다중 인스턴스 동시경합 부하테스트는 아직 없음(단일 순차 160건 실측만 함) |
| 7 | 운영 안정성 | 97 | −3: 콜드스타트 완화(`minInstances`) 대표가 명확히 거부 확정(상시비용 대비 불필요 판단) — 변동 없음 |
| 8 | 보안 | 97 | −3: Firestore 트랜잭션 자체의 동시경합 부하테스트 미실시(6번과 원인 공유). 부스토큰 노출·재생성제한 관련 사유는 전부 해소/종결됨 |
...
**미해결이고 코드 세션이 바로 처리 가능한 것(승인만 있으면 즉시 진행)**: 6·8번의 Firestore 트랜잭션 동시경합(진짜 동시요청) 부하테스트 — 지금까지는 순차 160건만 실측했고 진짜 동시성 테스트는 아직 안 함, 실비용 거의 없음.
