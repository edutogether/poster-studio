/* ────────────────────────────────────────────────────────────────────
   ...
   ──────────────────────────────────────────────────────────────────── */
/* 🔴 2026-09-09 리액트+TS 머지 이후 **이 파일은 더 이상 돌지 않는다.**
   번들로 합쳐지면서 `/api.js`·`/state.js` URL이 사라졌다. 지우지 않고 두는 이유는
   `snapshots/poster-pixels-*.json`(서브셋 전/후 20장 대조)이 이 코드로 뜬 값이라,
   그 숫자가 어떻게 나온 것인지 설명하는 기록이기 때문이다.
   **지금 쓸 것은 `poster-pixels-ui.js`다** — 진짜 버튼만 눌러서 번들에서도 돈다. */
