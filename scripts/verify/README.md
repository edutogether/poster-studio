...
`step2-<뷰포트>.json` 네 개가 **지금 유효한 기준선**이다(3단계 React 대조 대상).

- `-TOOLV1` 이 붙은 것은 `snapshot.js`의 `ownText`를 고치기 전(2026-09-09) 도구로 찍은 것이다.
  **도구 버전이 다른 스냅샷끼리 비교하지 말 것** — 키 규칙이 달라 전부 element-missing/added로
  나와 신호가 묻힌다. 지우지 않고 남긴 건 그때의 값이 기록으로 필요해서다.
- `-BADCAPTURE-` 는 촬영 절차를 틀린 것이다(아래 "촬영 절차 규칙" 참고). 역시 비교용이 아니라 증거다.
...
**공백뿐인 텍스트 노드는 지문에서 제외한다**(2026-09-09). 예전엔 그것들을 남겨서
`main.app|"    "`처럼 **소스 들여쓰기가 키에 박혀 있었다.** 마크업을 JSX로 옮기면
요소 사이 공백 줄이 사라져 `main.app`·`section.left` 같은 **컨테이너 키가 통째로 어긋나고**,
하필 레이아웃이 가장 중요한 자리들이 비교 대상에서 빠져 그 구멍으로 진짜 회귀가 샰다.
고친 뒤 현재 코드로 네 상태를 다시 찍어 **키 20개가 1:1로 이름만 바뀌고 공통 키의
내용 차이는 0건**임을 확인했고, 살아있는 DOM에 결함 4종(id 유실·요소 유실·진짜 텍스트
변경·속성 변경)을 심어 **전부 잡히는 것**을 확인했다(§5.4).

단, 키가 같아진 형제(예: `.panel` 둘)는 등장 순서 접미사(`#2`)로 갈린다 — 순서가
바뀌면 잘못 짝어질 수 있다. 다만 그 경우 위치·크기 차이가 크게 나기 때문에 놓치지는
않는다. 이 잔여 약점을 알고 쓴다.

**숨겨진 요소는 아예 수집되지 않는다**(`isVisible` 거름). 즉 `.hidden`인 `#snapshot`의
`alt` 같은 값은 **빈 화면 기준선에서는 검사되지 않는다** — `filled` 기준선이 필수인
이유가 하나 더 있다(실제로 변형 시험 중에 이 점을 확인했다).
