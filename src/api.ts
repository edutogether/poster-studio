...
$('generateBtn').onclick = async () => {
  ...
  if(!state.capturedBlob){ setStatus('먼저 사진을 촬영해 주세요.'); return; }
  ...
  form.append('photo', state.capturedBlob, 'capture.jpg');
...
$('regenBtn').onclick = () => { if(state.posters.length) $('generateBtn').click(); };
$('fallbackBtn').onclick = async () => {
  if(!state.pendingMeta || isGenerating) return;
  $('fallbackBtn').classList.add('hidden');
...
export function renderGallery(){
  ...
  state.posters.forEach((p,i) => {
    ...
    const t = document.createElement('div'); t.className='thumb'+(i===state.selected?' active':'');
