...
export const TEMPLATES: Template[] = [
  ...
  { label:'클래식', render(ctx,art,m,g){
    ...
    ctx.font="800 38px 'PretendardFull'"; ctx.fillText(FEST, W/2, 1066); setLS(ctx,0);
    const yEnd = drawTitle(ctx, `${m.title}`, g.font, W/2, 1212, 1000, 130, 60, '#fff');
    ctx.fillStyle=g.accent; setFitFont(ctx,800,46,"'PretendardFull'",creditMain(m),1080); ctx.fillText(creditMain(m), W/2, Math.max(yEnd+40,1360));
    const sub=creditSub(m); let y=Math.max(yEnd+40,1360);
    if(sub){ ctx.fillStyle='#dfe5f3'; setFitFont(ctx,600,32,"'PretendardFull'",sub,1080); ctx.fillText(sub, W/2, y+50); y+=50; }
    ctx.strokeStyle='rgba(255,255,255,.18)'; ctx.lineWidth=2; ctx.beginPath(); ctx.moveTo(300,y+95); ctx.lineTo(900,y+95); ctx.stroke();
    ctx.fillStyle='#fff'; ctx.font="800 42px 'PretendardFull'"; ctx.fillText(`${DATE}   ${VENUE}`, W/2, y+155);
  ...
  { label:'임팩트', render(ctx,art,m,g){
    ...
    const tg = `“${m.tagline}”`;
    ctx.textAlign='center'; ctx.fillStyle='#fff'; setFitFont(ctx,700,36,"'PretendardFull'",tg,1060);
    ...
    ctx.fillStyle='#fff'; setFitFont(ctx,800,40,"'PretendardFull'",creditMain(m),1080); ctx.fillText(creditMain(m), W/2, endY+L.size*0.7+30);
    ...
    ctx.fillStyle='#2a1d00'; setFitFont(ctx,800,30,"'PretendardFull'",barLine,1140);
  ...
  { label:'시네마', render(ctx,art,m,g){
    ...
    // 상단 띠: 영화제명
    ctx.textAlign='center'; ctx.fillStyle=g.accent; setLS(ctx,3); ctx.font="800 38px 'PretendardFull'"; ctx.fillText(FEST, W/2, 104); setLS(ctx,0);
    ...
    ctx.fillStyle=g.accent; setFitFont(ctx,800,36,"'PretendardFull'",creditMain(m),1060); ctx.fillText(creditMain(m), W/2, yEnd+28);
    ctx.fillStyle='#cfd6e8'; ctx.font="700 30px 'PretendardFull'"; ctx.fillText(`${DATE}   ${VENUE}`, W/2, yEnd+70);
  ...
  { label:'포토카드', render(ctx,art,m,g){
    ...
    ctx.font="800 32px 'PretendardFull'"; ctx.fillText(FEST, W/2, 1356); setLS(ctx,0);
    ...
    if(sub){ ctx.fillStyle='#41506f'; setFitFont(ctx,500,27,"'PretendardFull'",sub,980); ctx.fillText(sub, W/2, y+44); y+=44; }
    ctx.fillStyle='#41506f'; ctx.font="700 33px 'PretendardFull'"; ctx.fillText(`${DATE}   ·   ${VENUE}`, W/2, y+58); y+=58;
